...
import { getAll as getExpensesService } from '../services/expenseService';
...
export const InventoryProvider = ({ children }) => {
  ...
  const loadActiveCashSession = async () => {
    try {
      ...
      setCashDrawer(data); 
    } catch (error) { console.error("Error cargando caja activa:", error); }
  };

  const startCashSession = async (initialCash) => {
    try {
      await fetch(`${API_URL}/start`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ initialCash: parseFloat(initialCash) }) });
  ...
  const addProduct = async (productData) => { 
      try {
        ...
        setProducts(prevProducts => {
            ...
            if (alreadyExists) return prevProducts.map(p => p.id === savedProduct.id ? savedProduct : p);
            else return [...prevProducts, savedProduct];
        });
        return true;
      } catch (error) { console.error(error); return false; }
  };
  ...
  const addSale = async (cart, total, customer, seller, paymentMethod, ticketNumber, amountPaid, change, tip = 0) => {
    try { 
        const saleData = { cart, total, customer, seller, paymentMethod, ticketNumber, amountPaid: amountPaid !== undefined ? amountPaid : total, change: change !== undefined ? change : 0, tip: parseFloat(tip) };
        await createSaleService(saleData); 
        await loadProducts(); await loadSales(); await loadActiveCashSession(); 
        return true; 
    } catch (error) { console.error("Error al procesar venta:", error); alert("Error al registrar la venta."); return false; }
  };
  ...
  const addStockToProduct = async (id, stockData) => {
    ...
    } catch (error) { console.error("Error al agregar stock:", error); throw error; }
  };

  return (
    <InventoryContext.Provider value={{ 
      products, sales, cashDrawer, expenses, 
      startCashSession, closeCashSession, loadActiveCashSession, addProduct, updateProduct, deleteProduct, addSale, loadSales, loadExpenses, addStockToProduct
    }}>
      {children}
